...
def count_safe_reports(data: list[list[int]]):
    ...
    """
    count = 0
    firstSort = [report for report in data if sorted(report) == report or sorted(report, reverse = True) == report]
    return sum(1 if all(1 <= abs(a-b) <= 3 for a,b in zip(report, report[1:])) else 0 for report in firstSort)